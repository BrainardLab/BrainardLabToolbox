function addMondrianPar(GLWObj, numRows, numColumns, dimensions, colors, varargin)
...
switch obj.Type
	...
	% The horizontal shift amount is the width of 1.5 columns.
	shiftAmount = obj.Verts(1,2,1) - obj.Verts(1,1,1);
	
	offset = 0;
			offset = offset + shiftAmount + (.5*shiftAmount);

		
		obj.Verts(1,:,1) = obj.Verts(1,:,1) + offset;
	...
	offset = 0;
		...
		obj.Verts(5,:,1) = obj.Verts(5,:,1) + offset;
		obj.Verts(4,:,1) = obj.Verts(5,:,1) + offset;
		obj.Verts(3,:,1) = obj.Verts(4,:,1) + offset;
		obj.Verts(2,:,1) = obj.Verts(3,:,1) + offset;
		obj.Verts(1,:,1) = obj.Verts(2,:,1) + offset;
