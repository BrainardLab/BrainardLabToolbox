function addDotSet(GLWObj, dotPositions, rgbColor, dotSize, varargin)
% addDotSet - Adds a set of dots to the GLWindow.
%
% Syntax:
% obj.addDotSet(dotPositions, rgbColor, dotSize, [dotSetOpts])
%
% Input:
% dotPositions (Mx2|Mx3) - List of the (x,y) or (x,y.z) centers of each dot.
%    Each row of the matrix is an individual dot's position.
% rgbColor (1x3|struct|cell array) - RGB color for the dot set.
% dotSize (scalar) - Size of each dot in pixels.  All dots will be the
%    same size.
...
obj.ObjectType = GLWindow.ObjectTypes.DotSet;
