function showStereoImage2(imL,imR)
...
%
    ...
    % Clear everything
    clc;clear Classes; close all; ClockRandSeed;
...
%         userSelection = input('0 for Spheres (Achrom Forced Choice) scene),   1 for Cube scene : ');
        ...
        whichImage = 1;
        stereoPair.imageNames.left  = imL;%fullfile('StereoPairsRepository', [imageFiles{whichImage} 'L-RGB.mat']);
        stereoPair.imageNames.right = imR;%fullfile('StereoPairsRepository', [imageFiles{whichImage} 'R-RGB.mat']);
        
        % Load the left image data
        load(stereoPair.imageNames.left);
        if (whichImage == 1)
            stereoPair.imageData.left  = flipdim(RGB,1);
        else
        ...
        if (whichImage == 1)
            stereoPair.imageData.right  = flipdim(RGB,1);
        else
            stereoPair.imageData.right  = flipdim(sensorImageRGB,1);
        end
        
        % load the image width and height (in pixels)
        imageInfo.Height = 960;
        imageInfo.Width  = 1280;
        
        fov=26.6091;
    
    widthToHeightRatio     = imageInfo.Width/imageInfo.Height;
    
    imageSizeCm(1) = NaN;
    imageSizeCm(2) = ImageSizeFromAngle(fov, imageInfo.Height)/10; % this is image height
    imageSizeCm(1) = imageSizeCm(2)*widthToHeightRatio;
   ...
   % expConfigParams = convertToStruct(cfgFile);
    ...
    % pair images
    stereoPair.imagePosition  = [0,7];
    stereoPair.imageSize      = imageSizeCm;
