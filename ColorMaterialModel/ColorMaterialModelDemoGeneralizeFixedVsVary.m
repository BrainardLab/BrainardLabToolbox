% ColorMaterialModelDemoFixedVsVary
%
...
%% This is demo code. Simulate up some data, then analyze them. 
...
% We will set up a series of fixed weigths to loop through. 
% We will also simulate data with different underlying weigths. 
...
for ww = 1:length(simulateWeigth)
    for www = 1:length(fixedWValue)
        ...
        [returnedParams, logLikelyFit{ww, www}, predictedProbabilitiesBasedOnSolution, k] = FitColorMaterialModelMLDS(...
        ...
        % Compute RMSEs (a different way of gauging an error). 
        rmse{ww, www} = ComputeRealRMSE(probabilitiesFromSimulatedData, predictedProbabilitiesBasedOnSolution);
