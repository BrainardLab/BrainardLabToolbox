...
%    
    ...
    GUI.leftImageHandle = axes('Units','normalized','Position',[0.04 0.53 0.41 0.38]);
    
    % Create right image plot axes
    GUI.rightImageHandle = axes('Units','normalized','Position',[0.04 0.06 0.41 0.38]);
    
    % Initialize image plots
    InitializeImagePlots();
    
     % Create the X-Z top view plot
    GUI.XZplotHandle = axes('Units','normalized','Position',[0.57,0.06, 0.38, 0.86]);
    ...
    newRightImageButton = uicontrol( 'Style','pushbutton','String','Update Right Screen Image',...
                                'Units','normalized', ...
                                'Position', [0.17, 0.46, 0.145, 0.038], ...
                                'FontName', 'Helvetica', 'FontSize', 16, ...
                                'Callback',{@UpdateScreenImage_Callback});
                           
    % Create the Export button
    exportButton = uicontrol( 'Style','pushbutton','String','Export to PDF',...
                                'Units','normalized', ...
                                'Position', [0.47, 0.71, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16, ...
                                'Callback',{@Export_Callback});
          
    % Create the RRemove all button
    restartButton = uicontrol( 'Style','pushbutton','String','Remove all',...
                                'Units','normalized', ...
                                'Position', [0.47, 0.40, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16, ...
                                'Callback',{@Restart_Callback});
                            
    % Create the New Quad button
    newQuadButton = uicontrol( 'Style','pushbutton','String','New (L,R) quad',...
                                 'Units','normalized', ...
                                'Position', [0.47, 0.35, 0.082,0.038], 'FontName', 'Helvetica', 'FontSize', 16, ...
                                'Callback',{@NewQuad_Callback});
                                 
    % Create the New (L,R) Point button
    newPointButton = uicontrol( 'Style','pushbutton','String','New (L,R) point',...
                                 'Units','normalized', ...
                                'Position', [0.47, 0.30, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16, ...
    ...
    GUI.leftImagePointHandle  = uicontrol('Style','edit','String','', 'Enable', 'off', ...
                                 'Units','normalized', ...
                                'Position',[0.47, 0.25, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16);
    GUI.rightImagePointHandle = uicontrol('Style','edit','String','',  'Enable', 'off', ...
                                 'Units','normalized', ...
                                'Position',[0.47, 0.22, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16);
    
    % Create the 3D coords button (inactive)
    virtual3DButton = uicontrol( 'Style','pushbutton','String','3DPoint Coords',...
                                'Units','normalized', ...
                                'Position', [0.47, 0.15, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16, ...
                                'Enable', 'off');
    ...
    GUI.virtual3DPointXcoordHandle = ...
                    uicontrol('Style','edit','String','', 'Enable', 'off', ...
                               'Units','normalized', ...
                               'Position',[0.47, 0.125, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16);
    GUI.virtual3DPointYcoordHandle = ...
                    uicontrol('Style','edit','String','', 'Enable', 'off', ...
                              'Units','normalized', ...
                              'Position',[0.47, 100, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16);
    GUI.virtual3DPointZcoordHandle = ...
                    uicontrol('Style','edit','String','', 'Enable', 'off', ...
                              'Units','normalized', ...
                              'Position',[0.47, 0.096, 0.082, 0.038], 'FontName', 'Helvetica', 'FontSize', 16);
                                                                                
    % Create the Track XZpoint toggle button
    dataCursorModeButton = ...
                    uicontrol( 'Style','togglebutton','String','Data Cursor Mode',...
                                'Units','normalized', ...
                               'Position', [0.712, 0.923,0.095,0.038], 'FontName', 'Helvetica', 'FontSize', 16, ...
...
function InitializeImagePlots()
    ...
    xaxis = ([1:1920]-1920/2)/(1920)*virtualSceneWidthInCm;
    [xaxis(1) xaxis(end)]
    yaxis = ([1:1200]-1200/2)/(1200)*virtualSceneHeightInCm;
    zaxis = [0:120];
...
function UpdateImagePlots(whichScreen)
    ...
    xaxis = ([1:imageWidthInPixels]-imageWidthInPixels/2)/(imageWidthInPixels)*virtualSceneWidthInCm;
    yaxis = ([1:imageHeightInPixels]-imageHeightInPixels/2)/(imageHeightInPixels)*virtualSceneHeightInCm;
    ...
    if strcmpi(whichScreen, 'left')
        ...
        axis(GUI.leftImageHandle, 'image');
        box(GUI.leftImageHandle, 'on');
        ylabel(GUI.leftImageHandle, 'LCD display y-coord (cm)');
    else
        box(GUI.rightImageHandle, 'off');
        ...
        axis(GUI.rightImageHandle, 'image');
        box(GUI.rightImageHandle, 'on');
        xlabel(GUI.rightImageHandle, 'LCD display x-coord (cm)');
        ylabel(GUI.rightImageHandle, 'LCD display y-coord (cm)');
    end
...
function Export_Callback(varargin)
    ...
    [imageFileName, imageDirectory] = uiputfile('*.pdf','EPS filename', imageDirectory);
...
%     print(GUI.figHandle,'-depsc',fullfile(imageDirectory, imageFileName));
    ...
    handle = GUI.figHandle;
    pdfFileName = fullfile(imageDirectory, imageFileName);
    ...
    fprintf('\nPDF file saved in %s\n', fullfile(imageDirectory, imageFileName));
