...
while true
	whichHDRDisplay = GetInput('Calibrate left or right HDR display', 'string');
	switch whichHDRDisplay
		case 'left'
			frontScreenID = 4;
			backScreenID = 3;
			
			% Blank the other screens.
			mglSwitchDisplay(2);mglOpen(2);mglClearScreen(0);mglFlush;
			mglSwitchDisplay(5);mglOpen(5);mglClearScreen(0);mglFlush;
			break;
		case 'right'
			frontScreenID = 2;
			backScreenID = 5;
			...
			% Blank the other screens.
			mglSwitchDisplay(4);mglOpen(4);mglClearScreen(0);mglFlush;
			mglSwitchDisplay(3);mglOpen(3);mglClearScreen(0);mglFlush;
			break;
		otherwise
			fprintf('** Must specify "left" or "right"\n');
	end
end

% Generate the Mondrian colors based on the selected config file.
HDRCal.frontCal = LoadCalFile(sprintf('StereoHDRFront-%s', whichHDRDisplay));
HDRCal.backCal = LoadCalFile(sprintf('StereoHDRBack-%s', whichHDRDisplay));
HDRCal.P_scatter = zeros(HDRCal.frontCal.S_device(3),1);
% HDRCal.frontCal = LoadCalFile(sprintf('StereoHDRFront%s-%s', params.conditionName, whichHDRDisplay));
% HDRCal.backCal = LoadCalFile(sprintf('StereoHDRBack%s-%s', params.conditionName, whichHDRDisplay));
...
if (strcmp(whichToDo,'both') || strcmp(whichToDo,'frontonly'))
	...
	cal.describe.monitor = sprintf('StereoHDRMondrianFront%s-%s', params.conditionName, whichHDRDisplay);
	cal.describe.comment = sprintf('Stereo HDR Mondrian front %s %s (LCD) screen standard', params.conditionName, whichHDRDisplay);
	newFileName = sprintf('StereoHDRMondrianFront%s-%s', params.conditionName, whichHDRDisplay);
    ...
    cal.describe.yoked_name = sprintf('StereoHDRFrontYokedMondrian%s-%s', cal.mondrian.condition, whichHDRDisplay);
...
if (strcmp(whichToDo,'both') || strcmp(whichToDo,'backonly'))
	...
	cal.describe.meterDistance = 0.75;
	cal.describe.monitor = sprintf('StereoHDRMondrianBack%s-%s', params.conditionName, whichHDRDisplay);
	cal.describe.comment = sprintf('Stereo HDR Mondrian back %s %s (projector) screen standard', params.conditionName, whichHDRDisplay);
	newFileName = sprintf('StereoHDRMondrianBack%s-%s', params.conditionName, whichHDRDisplay);
    ...
    cal.describe.yoked_xy = [params.target_x ; params.target_y];
    cal.describe.yoked_nTargets = params.nTargets;
    cal.describe.yoked_name = sprintf('StereoHDRBackYokedMondrian%s-%s', cal.mondrian.condition, whichHDRDisplay);
...
if (doYoked)
	...
	HDRCal.frontCal = LoadCalFile(sprintf('StereoHDRFrontMondrian%s-%s', params.conditionName, whichHDRDisplay));
	HDRCal.backCal = LoadCalFile(sprintf('StereoHDRBackMondrian%s-%s', params.conditionName, whichHDRDisplay));
	HDRCal.P_scatter = zeros(HDRCal.frontCal.S_device(3),1);
	...
	for whichCalIndex = 1:2
		...
		switch (whichCal)
			case 'front'
				cal = LoadCalFile(sprintf('StereoHDRFrontMondrian%s-%s', params.conditionName, whichHDRDisplay));
			case 'back'
				cal = LoadCalFile(sprintf('StereoHDRBackMondrian%s-%s', params.conditionName, whichHDRDisplay));
			otherwise
	...
	sendmail('radonjic@sas.upenn.edu', 'Stereo HDR Calibration Complete', 'I am done.');
	...
	setpref('Internet', 'SMTP_Server', 'smtp-relay.upenn.edu');
	setpref('Internet', 'E_Mail', 'radonjic@sas.upenn.edu');
	sendmail('radonjic@sas.upenn.edu', 'Stereo HDR Calibration Error', e.message);
