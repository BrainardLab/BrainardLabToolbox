...
% 3/8/11   dh, mm  Put in code to calibrate back projector with r=g=b.
...
while true
	whichHDRDisplay = GetWithDefault('Calibrate left or right HDR display', 'left');
...
try
	...
	if (strcmp(whichToDo,'both') || strcmp(whichToDo,'frontonly'))
		...
		cal.describe.monitor = sprintf('StereoHDRFront-%s', whichHDRDisplay);
		cal.describe.comment = sprintf('Stereo HDR front %s (LCD) screen standard', whichHDRDisplay);
		newFileName = sprintf('StereoHDRFront-%s', whichHDRDisplay);
	...
	if (strcmp(whichToDo,'both') || strcmp(whichToDo,'backonly'))
		...
		cal.describe.meterDistance = 0.5;
		cal.describe.monitor = sprintf('StereoHDRBack-%s', whichHDRDisplay);
		cal.describe.comment = sprintf('Stereo HDR back %s (projector) screen standard', whichHDRDisplay);
		newFileName = sprintf('StereoHDRBack-%s', whichHDRDisplay);
		
		% This variable specifies whether we are doing some
		% special tricks to handle the HDR display, or not.
		%
		% Also force fgColor to be zero -- this is necessary
		% given the way we hacked up mglCalibrateMonDrvr to handle
		% the 'r=g=b' case.
		cal.fgColor = [0 ; 0 ; 0];
		cal.describe.HDRProjector = 'r=g=b';
	...
	if (doYoked)
		...
		for whichCalIndex = 1:2
			...
			switch (whichCal)
				case 'front'
					cal = LoadCalFile(sprintf('StereoHDRFront-%s', whichHDRDisplay));
				case 'back'
					cal = LoadCalFile(sprintf('StereoHDRBack-%s', whichHDRDisplay));
				otherwise
	...
	sendmail(emailTo, 'StereoHDR Calibration Complete', 'I am done.');
	...
	setpref('Internet', 'SMTP_Server', 'smtp-relay.upenn.edu');
	setpref('Internet', 'E_Mail', 'radonjic@sas.upenn.edu');
	sendmail(emailTo, 'StereoHDR Calibration Error', e.message);
